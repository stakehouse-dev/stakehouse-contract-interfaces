...
interface ISavETHManager {
    /// @notice For validators that have exited the beacon chain but have savETH dormant in an index, allow pushing into open index
    function pushSavETHIntoOpenIndex(
        address _stakeHouse,
        bytes calldata _blsPublicKeyMemberId
    ) external;
